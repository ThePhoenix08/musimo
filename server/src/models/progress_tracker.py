...
class ProgressTracker:
    ...
    def __init__(
        ...
        """
        self.steps = {
            step['id']: PipelineStep(id=step['id'], name=step['name'])
            for step in steps
        }
    ...
    async def start_step(self, step_id: str, message: Optional[str] = None):
        ...
        await self._emit_update('step_started', {
            'step': step.to_dict(),
    ...
    async def update_progress(
        ...
        self.overall_progress = (completed_steps / total_steps * 100) + (current_step_contribution * 100)
        
        await self._emit_update('progress_update', {
            'step': step.to_dict(),
            'all_steps': [s.to_dict() for s in self.steps.values()]
        })
    ...
    async def complete_step(
        ...
        await self._emit_update('step_completed', {
            'step': step.to_dict(),
            'all_steps': [s.to_dict() for s in self.steps.values()]
        })
    ...
    async def fail_step(self, step_id: str, error: str):
        ...
        await self._emit_update('step_failed', {
            'step': step.to_dict(),
            'error': error,
            'all_steps': [s.to_dict() for s in self.steps.values()]
        })
    ...
    async def skip_step(self, step_id: str, reason: str):
        ...
        await self._emit_update('step_skipped', {
            'step': step.to_dict(),
            'reason': reason,
            'all_steps': [s.to_dict() for s in self.steps.values()]
        })
    ...
    async def complete_pipeline(self, result: Any = None):
        ...
        await self._emit_update('pipeline_completed', {
            'all_steps': [s.to_dict() for s in self.steps.values()],
            'total_duration': time.time() - self.start_time,
            'result': result
        })
    ...
    async def fail_pipeline(self, error: str):
        ...
        await self._emit_update('pipeline_failed', {
            'error': error,
            'all_steps': [s.to_dict() for s in self.steps.values()],
            'total_duration': time.time() - self.start_time
        })
