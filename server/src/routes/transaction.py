...
async def get_transaction_summary(current_user: Dict = Depends(get_current_user)):
    ...
    try:
        ...
        emotion_count = sum(1 for t in transactions if t['model_type'] == 'emotion_detection')
        instrument_count = sum(1 for t in transactions if t['model_type'] == 'instrument_classification')
        
        emotion_predictions = [t['output']['prediction'] for t in transactions if t['model_type'] == 'emotion_detection']
        instrument_predictions = [t['output']['prediction'] for t in transactions if t['model_type'] == 'instrument_classification']
        
        from collections import Counter
        most_common_emotion = Counter(emotion_predictions).most_common(1)[0] if emotion_predictions else None
        most_common_instrument = Counter(instrument_predictions).most_common(1)[0] if instrument_predictions else None
        ...
        return {
            ...
            "most_common_instrument": most_common_instrument[0] if most_common_instrument else None
        }
