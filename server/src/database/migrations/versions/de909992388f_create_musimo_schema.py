...
def upgrade() -> None:
    ...
    sa.Column('source_type', sa.Enum('ORIGINAL', 'VOCALS', 'DRUMS', 'BASS', 'OTHER', name='audiosourcetype'), nullable=False),
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_audio_files_user_id_users'), ondelete='CASCADE'),
    ...
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('user_id', sa.UUID(), nullable=False),
    sa.ForeignKeyConstraint(['audio_id'], ['audio_files.id'], name=op.f('fk_analysis_jobs_audio_id_audio_files'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['model_id'], ['models.id'], name=op.f('fk_analysis_jobs_model_id_models'), ondelete='SET NULL'),
    ...
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.ForeignKeyConstraint(['audio_id'], ['audio_files.id'], name=op.f('fk_audio_features_audio_id_audio_files'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_audio_features')),
    sa.UniqueConstraint('id', name=op.f('uq_audio_features_id'))
    )
    op.create_index(op.f('ix_audio_features_audio_id'), 'audio_features', ['audio_id'], unique=False)
    ...
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.ForeignKeyConstraint(['audio_id'], ['audio_files.id'], name=op.f('fk_logs_audio_id_audio_files'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_logs')),
    sa.UniqueConstraint('id', name=op.f('uq_logs_id'))
    )
    op.create_table('separated_sources',
    ...
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.ForeignKeyConstraint(['parent_audio_id'], ['audio_files.id'], name=op.f('fk_separated_sources_parent_audio_id_audio_files'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_separated_sources')),
    sa.UniqueConstraint('id', name=op.f('uq_separated_sources_id'))
    )
    op.create_index(op.f('ix_separated_sources_parent_audio_id'), 'separated_sources', ['parent_audio_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_separated_sources_parent_audio_id'), table_name='separated_sources')
