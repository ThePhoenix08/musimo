...
class FeatureExtractor:
    ...
    def create_embedding_model(self, layer_name: str) -> tf.keras.Model:
        ...
        try:
            ...
            raise ValueError(f"Could not create embedding for layer '{layer_name}': {str(e)}")
    
    def get_embeddings(self, features: np.ndarray, layer_name: str = None) -> np.ndarray:
        """
        ...
        try:
            ...
            logger.info(f"Extracted embeddings from '{layer_name}': shape {embeddings.shape}")
            return embeddings
